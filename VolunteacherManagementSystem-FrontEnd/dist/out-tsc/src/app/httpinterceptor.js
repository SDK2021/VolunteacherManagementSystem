var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Injectable } from "@angular/core";
import { authentication } from './home/shared-services/authentication.service';
var httpinterceptor = /** @class */ (function () {
    function httpinterceptor(_auth) {
        this._auth = _auth;
    }
    httpinterceptor.prototype.intercept = function (req, next) {
        if (this._auth.isUserLogin()) {
            var token = localStorage.getItem(this._auth.LOCAL_STORAGE_ATTRIBUTE_USERNAME);
            var string = token.split(" ");
            console.log(atob(string[1]));
            req = req.clone({
                setHeaders: {
                    'authorization': "Basic " + btoa(atob(string[0]) + ":" + atob(string[1]))
                }
            });
        }
        return next.handle(req);
    };
    httpinterceptor = __decorate([
        Injectable(),
        __metadata("design:paramtypes", [authentication])
    ], httpinterceptor);
    return httpinterceptor;
}());
export { httpinterceptor };
//# sourceMappingURL=httpinterceptor.js.map